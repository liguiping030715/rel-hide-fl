...
def main():
    ...
    style()
    inputs = {
        "phase1": ROOT / "results/formal/scalability/protocol_consistent_release_v7_idempotent/formal_scalability_matrix.csv",
        "multiblock": ROOT / "results/formal/multiblock_scaling/protocol_consistent_release_v7_idempotent/formal_multiblock_scaling_matrix.csv",
        "baseline": ROOT / "results/formal/baselines/protocol_consistent_release_v7_idempotent_d784/controlled_baselines_matrix.csv",
        "shamir_proxy_baseline": ROOT / "results/formal/baselines/shamir_shuffle_proxy_d784/controlled_baselines_matrix.csv",
        "four_path_sum_only_baseline": ROOT / "results/formal/baselines/four_path_sum_only_d784/controlled_baselines_matrix.csv",
        "ablation": ROOT / "results/formal/ablations/protocol_consistent_release_v7_idempotent_c30_d3072/ablation_matrix.csv",
        "utility_summary": ROOT / "results/formal/fl_utility/protocol_consistent_release_v7_idempotent/summary.json",
    }
    phase1 = pd.read_csv(inputs["phase1"])
    multiblock = pd.read_csv(inputs["multiblock"])
    baseline_frames = [pd.read_csv(inputs["baseline"])]
    if inputs["shamir_proxy_baseline"].exists():
        baseline_frames.append(pd.read_csv(inputs["shamir_proxy_baseline"]))
    if inputs["four_path_sum_only_baseline"].exists():
        baseline_frames.append(pd.read_csv(inputs["four_path_sum_only_baseline"]))
    ...
    with open(inputs["utility_summary"], "r", encoding="utf-8") as f:
        utility_summary = json.load(f)
    utility = pd.DataFrame([
        ...
        for c in utility_summary["cases"]
    ])
    utility_round_paths = sorted((FORMAL / "fl_utility" / "protocol_consistent_release_v7_idempotent").glob("*_seed*/rounds.csv"))
    if not utility_round_paths:
        raise FileNotFoundError("No per-round FL utility CSV files found under results/formal/fl_utility.")
    ...
    comm_tbl["avg_total_MB"] = (comm_tbl["avg_total_bytes"] / (1024 * 1024)).round(2)
    comm_tbl["avg_bytes_per_client_MB"] = (comm_tbl["avg_total_bytes"] / comm_tbl["clients"] / (1024 * 1024)).round(2)
    ...
    multi_tbl["avg_total_MB"] = (multi_tbl["avg_total_bytes"] / (1024 * 1024)).round(2)
    multi_tbl["peak_RSS_MB"] = (multi_tbl["peak_rss_kb"] / 1024).round(2)
    ...
    base_tbl["client_upload_MB"] = (base_tbl["avg_client_upload_bytes"] / (1024 * 1024)).round(2)
    base_tbl["path_to_cs_MB"] = (base_tbl["avg_path_to_cs_bytes"] / (1024 * 1024)).round(2)
    base_tbl["avg_total_MB"] = (base_tbl["avg_total_bytes"] / (1024 * 1024)).round(2)
    ...
    abl_tbl["avg_total_MB"] = (abl_tbl["avg_total_bytes"] / (1024 * 1024)).round(2)
    abl_tbl = abl_tbl.drop(columns=["avg_total_bytes"])
    artifacts["tables"]["ablations"] = save_table(
        abl_tbl,
        "table_ablation",
        "Ablation study at $c=30,d=3072$.",
        "tab:eval-ablation",
    )
    ...
    ax.set_xlabel("Clients")
    ax.set_ylabel("Relay payload (MB)")
    ax.set_title("(a) One-block client scaling")
    ax.grid(True, alpha=0.3)
    ax.legend(fontsize=7)
    for clients_value, g in multiblock_stats.groupby("clients"):
    ...
    axb.set_xlabel("DCRTPoly blocks")
    axb.set_ylabel("Relay payload (MB)")
    axb.set_title("(b) Multi-block scaling")
    axb.set_xticks(sorted(multiblock_stats["block_count"].unique()))
    axb.grid(True, alpha=0.3)
    axb.legend(fontsize=7)
    ...
    display_labels = {
        ...
        "openfhe_bgv_only": "OpenFHE BGV-only",
    ...
    ax.set_xlabel("Clients")
    ax.set_ylabel("Serialized payload (MB)")
    ...
    ax2.set_ylabel("Relay payload (MB)")
    ax2.set_xticks(x)
    ...
    manifest = {
        "schema": "evaluation_artifact_manifest_v1",
        "status": "PASS",
